    ...
    return vol_ptr->rd()
        ...
        .thenValue([this, vol_ptr, vol_req,
            ...
            for (auto& blkid : new_blkids) {
                ...
                auto status = write_to_index(vol_ptr, vol_req, start_lba, end_lba, blocks_info);
            ...
            auto key_size = sizeof(HBJournalKey) + csum_size + old_blkids_size;
            ...
            // New blkid's are written to journal by the write_journal. After journal flush, on_commit will be called
            // where we free the old blkid's and the write iscompleted.
            HBJournalKey hb_key{vol_req->lba, vol_req->nlbas, static_cast< uint16_t >(old_blkids.size())};
            auto key_buf = req->key_buf().bytes();
            std::memcpy(key_buf, &hb_key, sizeof(HBJournalKey));
            key_buf += sizeof(HBJournalKey);
...
void HomeBlocksImpl::on_write(int64_t lsn, const sisl::blob& header, const sisl::blob& key,
    ...
    if (msg_header->corrupted()) {
        LOGERROR("homeblks message header is corrupted with crc error lsn={} header={}", lsn, msg_header->to_string());
        if (repl_ctx) { repl_ctx->promise_.setValue(folly::makeUnexpected(VolumeError::CRC_MISMATCH)); }
        return;
    }
