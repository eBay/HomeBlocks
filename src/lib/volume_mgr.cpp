...
shared< hs_index_table_t > HomeBlocksImpl::recover_index_table(homestore::superblk< homestore::index_table_sb >&& sb) {
    auto pid_str = boost::uuids::to_string(sb->parent_uuid); // parent_uuid is the volume id
    {
        ...
        std::vector< chunk_num_t > chunk_ids(sb->get_chunk_ids(), sb->get_chunk_ids() + sb->num_chunks);
        bool success =
            index_chunk_selector_->recover_chunks(sb->ordinal, sb->pdev_id, 0, chunk_ids);
        if (!success) {
            LOGI("Failed to recover chunks for index table index_uuid: {}, parent_uuid: {} ordinal: {}", boost::uuids::to_string(sb->uuid), pid_str, sb->ordinal);
        }
