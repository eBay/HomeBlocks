...
struct HomeBlksMessageHeader : public BaseMessageHeader< HomeBlksMessageHeader > {
    ...
    bool corrupted() const {
        if (magic_num != HOMEBLKS_MSGHEADER_MAGIC || protocol_version != HOMEBLKS_MSGHEADER_PROTOCOL_VERSION_V1) {
            return true;
        }
    ...
    std::string to_string() const {
        return fmt::format(
            "magic={:#x} version={} msg_type={} payload_size={} payload_crc={} header_crc={} volume={}\n", magic_num,
            protocol_version, enum_name(msg_type), payload_size, payload_crc, header_crc,
            boost::uuids::to_string(volume_id));
    }
};
