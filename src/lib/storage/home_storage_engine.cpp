...
#define SM_STORE_LOG(level, msg, ...)                                                                                  \
    LOG##level##MOD_FMT(home_replication, ([&](fmt::memory_buffer& buf, const char* msgcb, auto&&... args) -> bool {   \
                            ...
                            fmt::vformat_to(fmt::appender{buf}, fmt::string_view{"[{}={}] "},                          \
                                            fmt::make_format_args("rs", m_sb_in_mem.gid));                             \
...
HomeStateMachineStore::HomeStateMachineStore(std::string const& rs_gid) : m_sb{"replica_set"} {
    LOGDEBUGMOD(home_replication, "Creating new instance of replica state machine store for gid={}", rs_gid);
...
HomeStateMachineStore::HomeStateMachineStore(const homestore::superblk< home_rs_superblk >& rs_sb) :
        m_sb{"replica_set"} {
    LOGDEBUGMOD(home_replication, "Opening existing replica state machine store for gid={}", rs_sb->gid);
    m_sb = rs_sb;
    m_sb_in_mem = *m_sb;
