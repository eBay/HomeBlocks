...
void HomeBlocksImpl::init_homestore() {
    ...
    volume_chunk_selector_ = std::make_shared< VolumeChunkSelector >("volume",
        [this](uint64_t volume_ordinal, const std::vector< chunk_num_t >& chunk_ids) {
            update_vol_sb_cb(volume_ordinal, chunk_ids);
        });
    LOGI("initi index chunkselc:");
    index_chunk_selector_ = std::make_shared< VolumeChunkSelector >("index",
         [this](uint64_t volume_ordinal, const std::vector< chunk_num_t >& chunk_ids) {
            // update_vol_sb_cb(volume_ordinal, chunk_ids);
         });
    LOGI("DUMPING index chunks:");
    index_chunk_selector_->dump_chunks();
    ...
    bool need_format = homestore::hs()
                           ->with_index_service(std::make_unique< HBIndexSvcCB >(this), index_chunk_selector_) // custom index chunk selector
    ...
    if (need_format) {
        ...
        if (has_data_dev && has_fast_dev) {
            ...
            homestore::hs()->format_and_start({
                ...
                {HS_SERVICE::INDEX, hs_format_params{.dev_type = HSDevType::Fast, .size_pct = 45.0, .chunk_sel_type = chunk_selector_type_t::CUSTOM}},
            ...
            homestore::hs()->format_and_start({
                ...
                {HS_SERVICE::INDEX, hs_format_params{.dev_type = run_on_type, .size_pct = 5.0, .chunk_sel_type = chunk_selector_type_t::CUSTOM}},
                {HS_SERVICE::REPLICATION,
