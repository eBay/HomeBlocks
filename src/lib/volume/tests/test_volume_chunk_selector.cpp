...
TEST_F(ChunkSelectorTest, AllocateReleaseChunksTest) {
    auto chunk_sel = std::make_shared< VolumeChunkSelector >("test", [this](uint64_t, const std::vector< chunk_num_t >&) {});
...
TEST_F(ChunkSelectorTest, ResizeNumChunksTest) {
    ...
    auto chunk_sel = std::make_shared< VolumeChunkSelector >("test",
