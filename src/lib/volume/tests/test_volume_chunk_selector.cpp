...
TEST_F(ChunkSelectorTest, AllocateReleaseChunksTest) {
    ...
    uint32_t pdevs = 3, num_chunks_per_pdev = 10;
    // Add chunks to chunk selector, each chunk is 16KB, so 3 * 10 * 16KB = 480KB
    ...
    auto vol1_chunks = chunk_sel->allocate_volume_chunks(0 /* ordinal */, 180 * Ki);
    ...
    for (uint32_t i = 1; i < 5; i++) {
        auto vol_chunks = chunk_sel->allocate_volume_chunks(i /* ordinal */, 180 * Ki);
    ...
    for (uint32_t i = 0; i < 5; i++) {
        chunk_sel->release_volume_chunks(i /* ordinal */);
    }
...
TEST_F(ChunkSelectorTest, SelectChunksTest) {
    ...
    auto active_chunks = chunk_sel->get_volume_chunks(0);
...
TEST_F(ChunkSelectorTest, ResizeNumChunksTest) {
    ...
    set_flip_point("vol_num_chunks_resize_op");

    // Add chunks to chunk selector, each chunk is 16KB, so 3 * 10 * 16KB = 480KB
    uint32_t pdevs = 3, num_chunks_per_pdev = 10;
    add_chunks_per_pdev(chunk_sel, pdevs, num_chunks_per_pdev);

    auto initial_chunk_ids = chunk_sel->allocate_volume_chunks(0 /* ordinal */, 180 * Ki);
    RELEASE_ASSERT(!initial_chunk_ids.empty(), "no chunks");
    auto initial_chunks = chunk_sel->get_volume_chunks(0);
    ...
    auto resized_chunks = chunk_sel->get_volume_chunks(0);
...
TEST_F(ChunkSelectorTest, RecoverChunksTest) {
    ...
    // Simulate a recovery.
    chunk_sel->recover_volume_chunks(0 /* ordinal*/, 180 * Ki, chunk_ids);
    homestore::blk_alloc_hints hints;
    hints.application_hint = 0;
    auto chunk = chunk_sel->select_chunk(1 /* nblks */, hints);
    RELEASE_ASSERT(chunk, "Chunk not available");
