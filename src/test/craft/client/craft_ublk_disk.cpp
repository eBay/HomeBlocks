...
// The per-IO worker: started on a worker reactor (so the CRAFT client op runs where async storage / network
// completions live), it co_awaits the client read/write and -- resuming when it completes -- hands the result
...
static async_status run_craft_io(std::shared_ptr< craft::craft_client > client, bool is_read, uint64_t addr,
    ...
    // Post the completion CQE directly onto the queue's ring. We're on a uring-capable reactor, so this is one
    // queued SQE that the reactor's poll_once batches out -- no eventfd, no lock, no service loop.
    if (auto const r = iomanager.post_msg_ring(queue_ring_fd, state_ud, result); r != 0) {
        ...
        LOGWARN("craft_ublk_disk: direct msg_ring post returned {}; retrying via a worker reactor", r);
        iomanager.run_on_forget(iomgr::reactor_regex::random_worker, [queue_ring_fd, state_ud, result]() {
            if (auto const r2 = iomanager.post_msg_ring(queue_ring_fd, state_ud, result); r2 != 0) {
                LOGERROR("craft_ublk_disk: msg_ring post failed on worker reactor ({}); IO {:#x} may stall", r2,
                         state_ud);
            }
        });
    }
...
CraftUblkDisk::prepare_result CraftUblkDisk::prepare(ublksrv_queue const*, int const) {
...
ublkpp::disk_task< int > CraftUblkDisk::async_iov(ublksrv_queue const* q, ublk_io_data const* data, iovec* iovecs,
    ...
    // Per-IO cqe_state in the tag's pool. `state_ud` is its managed-encoded pointer; the CRAFT completion carries
    // it back via msg_ring, and run_queue_loop decodes it to resume `state` (writing state->_result = cqe->res,
    // i.e. our IO result) -- all on this queue thread. We touch `state` only here and there: no lock.
